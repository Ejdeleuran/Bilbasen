...
# Vis de to højeste priser for hvert land
print("Top 2 priser i Danmark:")
print(top2_danish)

print("Top 2 priser i Tyskland:")
print(top2_german)
...
ggplot(price_comparison, aes(x = Country, y = Price, fill = Country)) +
  ...
  scale_fill_manual(values = c("Danmark - Top 1" = "#1f78b4", "Danmark - Top 2" = "#a6cee3",
                               "Tyskland - Top 1" = "#33a02c", "Tyskland - Top 2" = "#b2df8a")) +  # Farver
  scale_y_continuous(labels = comma) +  # Formatter y-aksen med tusindtalsseparator
  geom_text(aes(label = paste0(comma(round(Price, 0)), " DKK")), 
